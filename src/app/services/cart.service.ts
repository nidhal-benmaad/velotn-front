...
export class CartService {
   ...
   minShipping=0;
    ...
    add(product: Product, quantity: number, options: {name: string; value: string}[] = []): Observable<CartItem> {
        ...
        return timer(1000).pipe(map(() => {
            ...
            let item = this.items.find(eachItem => {
                if (eachItem.product._id !== product._id || eachItem.options.length !== options.length) {
                    return false;
                }
            ...
            if (item) {
               ...
               {
                    this.toastr.error( 'Vous ne pouvez pas ajouter plus d\'articles que ceux qui sont disponibles. En stock ' + product.stock + ' articles et vous avez déjà ajouté ' + item.quantity + ' articles à votre panier');
    ...
    public calcSub(item: CartItem): number {
        let quantity = 0;
        let subtotal = 0;
        this.data.items = this.data.items.filter(eachItem => eachItem !== item);
        this.data.items.forEach(item => {
            ...
            if(item.product.promo){
                subtotal += item.product.promo.price * item.quantity;
            }
            else subtotal += item.product.price * item.quantity;
